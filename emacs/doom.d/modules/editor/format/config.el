...
(defvar +format-on-save-context 'buffer
  "When a buffer is modified the format module can modify only the parts that
  have been modified part or larger enclosing contexts. The default is to
  reformat the entire buffer.

Possible values are `buffer' to reformat the entire buffer and `modification' to
